...
def app(db):
    ...
    connect_to_wlan(db.ssid, db.passwd)
    ...
    if (not (
        reset_cause == machine.PWRON_RESET or
        reset_cause == machine.HARD_RESET
        or (
    ...
    response = urequests.get(f'http://homeapi.antti.codes/electricity_prices?margin={db.margin}')
